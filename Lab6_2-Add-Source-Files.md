...
Multiple Source Files: เพื่อแยกหน้าที่ของแต่ละโมดูลให้โค้ดอ่านง่าย แก้ไขบำรุงรักษาได้สะดวก

2. **CMakeLists.txt Management**: การเพิ่มไฟล์ source ใหม่ต้องแก้ไขอะไรบ้าง?
CMakeLists.txt Management: ต้องเพิ่มชื่อไฟล์ .c ใหม่ในส่วน SRCS เพื่อให้ระบบ build รวมไฟล์นั้นเข้า binary.

3. **Header Files**: บทบาทของไฟล์ .h คืออะไร และทำไมต้องมี?
Header Files: ใช้ประกาศฟังก์ชัน ตัวแปร และโครงสร้างข้อมูล เพื่อให้ไฟล์อื่นสามารถเรียกใช้งานได้โดยไม่ซ้ำซ้อน.

4. **Include Directories**: เหตุใด CMakeLists.txt ต้องระบุ INCLUDE_DIRS?
Include Directories: เพื่อบอก CMake ว่าจะค้นหา header (.h) จากโฟลเดอร์ใดเวลาคอมไพล์.

5. **Git Ignore**: ไฟล์ .gitignore ช่วยอะไรในการจัดการ ESP32 project?
Git Ignore: ป้องกันไม่ให้ไฟล์ build, binary, และไฟล์ชั่วคราวถูก push ขึ้น Git repository โดยไม่จำเป็น.

6. **Task Management**: การใช้ FreeRTOS task ในโมดูล LED ช่วยอะไร?
Task Management: ช่วยให้ LED ทำงานแบบแยก thread จาก main ทำให้ระบบ multitasking และ responsive มากขึ้น.

7. **Code Organization**: ข้อดีของการแยกโมดูล sensor, display, led เป็นไฟล์แยกคืออะไร?
Code Organization: แยก sensor, display, led ทำให้แต่ละส่วนพัฒนา ทดสอบ และแก้ไขได้อิสระ ลดการซ้ำซ้อนของโค้ด.
...
- จำนวนไฟล์ source: 1 ไฟล์ (main.c + sensor.c)
- ขนาด binary: ประมาณ 210 KB
- การทำงาน: อ่านค่าจาก sensor และแสดงผลใน serial monitor.

**ขั้นตอนที่ 2 (เพิ่ม display.c):**
- จำนวนไฟล์ source: 2 ไฟล์ (sensor.c, display.c)
- ขนาด binary: ประมาณ 230 KB
- การทำงาน: อ่าน sensor และแสดงข้อมูลบนจอแสดงผล (เช่น OLED).

**ขั้นตอนที่ 3 (เพิ่ม led.c):**
- จำนวนไฟล์ source: 3 ไฟล์ (sensor.c, display.c, led.c)
- ขนาด binary: ประมาณ 250 KB
- การทำงาน: อ่าน sensor, แสดงผลบนจอ และควบคุม LED กระพริบผ่าน FreeRTOS task.
